...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['order_date'] = pd.to_datetime(df['order_date'], errors='coerce').fillna('1970-01-01')
    df['total_amount'] = pd.to_numeric(df['total_amount'], errors='coerce').fillna(0.0)
    df['total_amount'] = df['total_amount'].clip(lower=0.0)
    df['total_amount'] = df['total_amount'].clip(upper=100000.0)
    # Convert item_count to integer with clamping
    df['item_count_num'] = pd.to_numeric(df['item_count'], errors='coerce')
    df['item_count'] = df['item_count_num'].fillna(1).astype(int)
    
    df['item_count'] = df['item_count'].clip(lower=1)
    
    df['item_count'] = df['item_count'].clip(upper=100)
    
    df = df.drop(columns=['item_count_num'])
    df['total_amount'] = pd.to_numeric(df['total_amount'], errors='coerce').fillna(0.0)
...
def clean_and_load_pipeline(source_path: str = "/home/aq/Documents/Source/loops/data/orders.csv"):
    ...
    schema_path = Path(source_path).parent.parent / "schemas" / "orders_schema.yaml"
    try:
