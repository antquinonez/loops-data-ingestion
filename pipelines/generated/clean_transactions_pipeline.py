...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['amount'] = df['amount'].clip(upper=10000.0)
    df['date'] = pd.to_datetime(df['date'], errors='coerce').fillna('1970-01-01')
...
def clean_and_load_pipeline(source_path: str = "/home/aq/Documents/Source/loops/data/transactions.csv"):
    ...
    schema_path = Path(source_path).parent.parent / "schemas" / "transactions_schema.yaml"
    try:
